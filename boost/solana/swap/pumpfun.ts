...
import { JITO_MODE, SLIPPAGE } from "../../../utils/constant"
import { Blockhash } from "../../../utils/types"
import { execute } from "../executor/legacy"
import { executeJitoTx } from "../executor/jito"

export const getBuyTxWithJupiter = async (wallet: Keypair, baseMint: string, amount: number) => {
  try {
    const quoteResponse = await (
      await fetch(
        `https://quote-api.jup.ag/v6/quote?inputMint=So11111111111111111111111111111111111111112&outputMint=${baseMint}&amount=${amount}&slippageBps=${SLIPPAGE}`
      )
    ).json();
